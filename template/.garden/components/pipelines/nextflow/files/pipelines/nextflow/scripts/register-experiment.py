...
Manages SQLite database for Nextflow pipeline experiment tracking
"""
...
class ExperimentRegistry:
    """Manages experiment database operations"""
    ...
    def create_experiment(
        ...
        try:
            ...
            print(f"❌ Experiment already exists: {exp_id}")
            return False
    ...
    def update_git_info(self, exp_id: str, git_commit: str, git_branch: str, is_dirty: bool = False) -> bool:
        """Update git information for experiment"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        try:
            cursor.execute("""
                INSERT INTO executions (
                    execution_id, experiment_id, status,
    ...
    def add_result(
        ...
        try:
            cursor.execute("""
                INSERT INTO results (
                    experiment_id, name, value, unit, category, file
                )
                VALUES (?, ?, ?, ?, ?, ?)
    ...
    def list_experiments(self, exp_type: Optional[str] = None, status: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def view_experiment(self, exp_id: str) -> Optional[Dict[str, Any]]:
        ...
        # Get executions
        cursor.execute("SELECT * FROM executions WHERE experiment_id = ?", (exp_id,))
        ...
        # Get tags
        cursor.execute("SELECT tag FROM experiment_tags WHERE experiment_id = ?", (exp_id,))
    ...
    def search_by_tag(self, tag: str) -> List[Dict[str, Any]]:
        ...
        cursor.execute("""
            ...
            JOIN experiment_tags t ON e.id = t.experiment_id
            WHERE t.tag = ?
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  create --id ID --type TYPE       Create experiment")
        ...
        print("  view --id ID                     View experiment details")
    ...
    if command == "init-db":
        ...
        exp = registry.view_experiment(args.get('id'))
        if exp:
            print(json.dumps(exp, indent=2))
        else:
            print(f"❌ Experiment not found: {args.get('id')}")
    
    elif command == "search-tag":
        experiments = registry.search_by_tag(args.get('tag'))
        print(f"\n🏷️  Found {len(experiments)} experiments with tag '{args.get('tag')}':\n")
        for exp in experiments:
