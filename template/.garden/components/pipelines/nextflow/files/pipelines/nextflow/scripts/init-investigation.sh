...
# Nextflow Investigation Initialization Script
# Creates new investigation directories and populates templates
...
TEMPLATES_DIR="${BASE_DIR}/investigations/templates"
CONFIGS_DIR="${BASE_DIR}/investigations/configs"
...
Required:
  --type TYPE               Investigation type: development, production, or planning
  --name NAME               Short name for the investigation
...
Optional:
  ...
  --parent-id ID            Parent investigation ID (for resume)
...
echo -e "${BLUE}🧪 Creating investigation: ${EXP_ID}${NC}"
...
if [[ -d "$EXP_DIR" ]]; then
    echo -e "${RED}Error: Investigation directory already exists: $EXP_DIR${NC}"
    exit 1
fi
...
substitute_template() {
    ...
    sed -e "s|{{INVESTIGATION_ID}}|${EXP_ID}|g" \
        -e "s|{{TYPE}}|${TYPE}|g" \
...
substitute_template "${TEMPLATES_DIR}/investigation-plan.md.template" "${EXP_DIR}/investigation-plan.md"
...
# Investigation Parameters: ${EXP_ID}
...
python3 "${SCRIPT_DIR}/register-investigation.py" create \
    --id "${EXP_ID}" \
    --type "${TYPE}" \
    --scenario "${SCENARIO}" \
    --phase "${PHASE}" \
...
ACTIVE_DIR="${BASE_DIR}/investigations/${TYPE}/active"
mkdir -p "${ACTIVE_DIR}"
...
echo ""
echo -e "${BLUE}════════════════════════════════════════${NC}"
echo -e "${GREEN}✅ Investigation created successfully!${NC}"
echo -e "${BLUE}════════════════════════════════════════${NC}"
echo ""
echo -e "  ${BLUE}Investigation ID:${NC} ${EXP_ID}"
echo -e "  ${BLUE}Type:${NC}          ${TYPE}"
echo -e "  ${BLUE}Scenario:${NC}      ${SCENARIO}"
echo -e "  ${BLUE}Location:${NC}      ${EXP_DIR}"
echo -e "  ${BLUE}Symlink:${NC}       investigations/${TYPE}/active/${NAME}"
echo ""
echo -e "${YELLOW}📝 Next Steps:${NC}"
echo -e "  1. Edit investigation plan: ${EXP_DIR}/investigation-plan.md"
